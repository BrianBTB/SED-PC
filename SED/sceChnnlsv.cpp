...
int sceSdGetLastIndex(u32 addressCtx, u32 addressHash, u32 addressKey)
{
    ...
    Memory::ReadStruct(addressCtx, &ctx);		//Read in the struct from addressCtx into ctx
    int res = sceSdGetLastIndex_(ctx, Memory::GetPointer(addressHash), Memory::GetPointer(addressKey));
    Memory::WriteStruct(addressCtx, &ctx);		//Write back the struct from ctx to addressCtx
    return res;
}
